                    ...
                    <div class="metro-registration-tool__hero">
                        <div class="metro-registration-tool__hero-copy">
                            <h2 class="metro-registration-tool__headline">
                                Find Your
                                <span>Registration &amp; Inspection Documents</span>
                            </h2>
                            <p class="metro-registration-tool__lede">
                                Look up a single trailer or pull documents for your whole fleet at once.
                            </p>
                            <p class="metro-registration-tool__lede metro-registration-tool__lede--detail">
                                Enter a trailer number to see matching results as you type, or paste a list of trailer numbers separated by commas to download all their registration and FHWA inspection PDFs in one batch. We'll show you which trailers we have documents for and flag any we can't find, so you know exactly what you're getting before you download.
                            </p>
                        </div>
                        <div class="metro-registration-tool__hero-card">
                            ...
                            <div class="metro-registration-tool__hero-stat">
                                ...
                                <strong>Start typing a trailer number and we'll suggest matches after the first few characters.</strong>
                            </div>
                            <div class="metro-registration-tool__hero-stat">
                                <span class="metro-registration-tool__hero-stat-label">Bulk download</span>
                                <strong>Paste your list and we'll gather every available document into one download.</strong>
                            </div>
                            <div class="metro-registration-tool__hero-stat">
                                <span class="metro-registration-tool__hero-stat-label">Smart file detection</span>
                                <strong>We automatically show only the documents that exist, and check availability for any trailers we haven't seen before when you click to download.</strong>
                            </div>
                        </div>
                    </div>
                    ...
                    <form class="metro-registration-tool__search" novalidate>
                        ...
                        <div class="metro-registration-tool__search-row">
                            <textarea
                                ...
                                rows="2"
                                maxlength="5000"
                                placeholder="5318190 or 5318190, 5327599, MTRZ281643"
                            ></textarea>
                        ...
                        <p class="metro-registration-tool__hint">
                            ...
                            and click Search to run a batch lookup.
                        </p>
            ...
            <script>
                (function () {
                    ...
                    function renderDocButton(folderId, documentType, label, available) {
                        ...
                        const pendingPhase = getPendingDocumentAction(folderId, documentType);
                        if (available === true || available === null) {
                            ...
                            return `<button class="metro-registration-tool__doc-button${loadingClass}" type="button" data-folder-id="${escapeHtml(folderId)}" data-document-type="${escapeHtml(documentType)}"${disabledAttr}>${escapeHtml(buttonLabel)}</button>`;
                        }
                                ...
                                <table class="metro-registration-tool__table">
                                    <thead>
                                        <tr>
                                            ...
                                            <th scope="col" class="metro-registration-tool__doc-heading">FHWA Inspection</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                    ...
                    function renderBatchResults(payload) {
                        ...
                        let html = `
                            <div class="metro-registration-tool__results-toolbar">
                                <div>
                                    <p class="metro-registration-tool__subheading">Batch lookup</p>
                                    <p class="metro-registration-tool__toolbar-copy">Found ${Number(payload.matched_count || 0).toLocaleString()} matches from ${Number(payload.requested_count || 0).toLocaleString()} trailer numbers.</p>
                                </div>
                                ${payload.can_download_all ? `<button class="metro-registration-tool__bulk-button${state.bulkDownloadPhase ? ' is-loading' : ''}" type="button"${state.bulkDownloadPhase ? ' disabled' : ''}>${escapeHtml(getBulkButtonLabel())}</button>` : '<p class="metro-registration-tool__toolbar-copy">Download all documents is available when fewer than 100 trailers are matched.</p>'}
                            </div>
                        `;
                        ...
                        if ((payload.missing || []).length > 0) {
                            html += `
                                <div class="metro-registration-tool__missing-card">
                                    ...
                                    <p class="metro-registration-tool__missing-note">We didn't find a match for these trailer numbers.</p>
                        ...
                        setStatus(`Checked ${Number(payload.requested_count || 0).toLocaleString()} trailer numbers. Found ${Number(payload.matched_count || 0).toLocaleString()} matches and ${Number(payload.missing_count || 0).toLocaleString()} without a match.`);
                    }
                    ...
                    async function performBatchSearch(query) {
                        ...
                        if (trailers.length === 0) {
                            clearResults();
                            setStatus('');
                            setError('Enter at least one trailer number for batch lookup.');
                            return;
                        }

                        setBusy(true);
                        setError('');
                        setStatus(`Looking up ${trailers.length} trailer numbers...`);
                    ...
                    input.addEventListener('input', function () {
                        ...
                        if (isBatchQuery(trimmed)) {
                            ...
                            clearDocuments();
                            setError('');
                            setStatus('Paste a comma-separated list and click Search to run a batch lookup.');
                            return;
                        }
