            ...
            <script>
                (function () {
                    ...
                    function getLoadingLabel(documentType, phase) {
                        if (phase === 'opening') {
                            return documentType === 'FHWA Inspection' ? 'Opening FHWA...' : 'Opening registration...';
                        }

                        return documentType === 'FHWA Inspection' ? 'Checking FHWA...' : 'Checking registration...';
                    }
                    ...
                    function renderBatchResults(payload) {
                        ...
                        let html = `
                            <div class="metro-registration-tool__results-toolbar">
                                ...
                                ${payload.can_download_all ? '<button class="metro-registration-tool__bulk-button" type="button">Download all documents</button>' : '<p class="metro-registration-tool__toolbar-copy">Bulk ZIP download is only available when fewer than 100 trailers are matched.</p>'}
                            </div>
                        `;
                    ...
                    async function downloadBulkDocuments() {
                        ...
                        setError('');
                        setStatus(`Preparing ${state.bulkFolderIds.length} trailer folders for download...`);
                        ...
                        try {
                            ...
                            setStatus(`Downloaded ${state.bulkFolderIds.length} trailer folders as a ZIP file.`);
                        } catch (requestError) {
