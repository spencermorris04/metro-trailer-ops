...
if (!class_exists('Metro_Registration_Graph_Client')) {
    class Metro_Registration_Graph_Client
    {
        ...
        private const DOCUMENT_SYNC_STATE_OPTION = 'metro_registration_lookup_tool_document_sync_state';
        private const DOCUMENT_SEED_WORKER_COUNT = 8;
        private const DOCUMENT_SEED_BATCH_SIZE = 30;
        private const DOCUMENT_DELTA_PAGE_LIMIT = 10;
        private const DOCUMENT_DELTA_RESCAN_LIMIT = 150;
        ...
        public function search_trailer_folders(string $query, int $limit = self::SEARCH_LIMIT_DEFAULT): array
        {
            ...
            $response = $this->build_unified_search_response($query, $limit, $asset_status, $status);

            if ($response['total_matches'] === 0) {
                $numeric_fallback = $this->build_numeric_search_fallback($query, 4);
                if ($numeric_fallback !== null) {
                    $fallback_response = $this->build_unified_search_response($numeric_fallback, $limit, $asset_status, $status);
        ...
        {
            return $this->get_trailer_documents_internal($folder_id, false);
        }
        ...
        public function lookup_exact_trailer_batch(array $queries): array
        {
            ...
            foreach ($queries as $query) {
                ...
                if ($matched_trailer === null) {
                    $matched_trailer = $this->resolve_live_exact_trailer($query['query']);
                }

                if ($matched_trailer === null && !empty($query['numeric_fallback'])) {
                    $matched_trailer = $this->resolve_live_exact_trailer($query['numeric_fallback']);
                }
        ...
        private function resolve_live_exact_trailer(string $query): ?array
        {
            $query = trim($query);
            if ($query === '') {
                return null;
            ...
            $asset_match = $this->fetch_business_central_exact_asset($query);
            $folder_match = $this->find_exact_folder_by_name($query, true);

            if ($asset_match !== null) {
                $folder = $folder_match ?: $this->find_best_folder_for_asset($asset_match, true);
                return $this->format_asset_result($asset_match, $folder);
            }

            if ($folder_match !== null) {
                $asset_from_folder = $this->fetch_business_central_exact_asset((string) ($folder_match['name'] ?? ''));
                if ($asset_from_folder !== null) {
                    return $this->format_asset_result($asset_from_folder, $folder_match);
                }

                return $this->format_legacy_folder_as_trailer($this->format_folder_result($folder_match));
            }
        ...
        public function download_registration_batch(array $folder_ids, array $trailers = []): array
        {
            $folder_ids = array_values(array_unique(array_filter(array_map('trim', $folder_ids), 'strlen')));
            if (empty($folder_ids)) {
            ...
            $folder_paths = [];
            $manifest_rows = [];
            $manifest_lookup = $this->build_trailer_manifest_lookup($trailers);
            ...
            try {
                foreach ($folder_ids as $folder_id) {
                    ...
                    $manifest_rows[] = $this->build_manifest_row_for_folder(
                        $documents['folder'],
                        $documents['documents'],
                        $manifest_lookup
                    );
                ...
                if (!empty($manifest_rows)) {
                    $manifest_path = trailingslashit($temp_dir) . 'trailer-manifest.csv';
                    $this->write_manifest_csv($manifest_path, $manifest_rows);
                    $written_files['trailer-manifest.csv'] = $manifest_path;
                }

                $export_files = $written_files;
                unset($export_files['download-notes.txt']);
                unset($export_files['trailer-manifest.csv']);
        ...
        public function get_document_seed_worker_ids(): array
        {
            return range(0, self::DOCUMENT_SEED_WORKER_COUNT - 1);
        }

        public function get_document_sync_status(): array
        {
            global $wpdb;

            $state = get_option(self::DOCUMENT_SYNC_STATE_OPTION, []);
            $table_name = self::get_index_table_name();
            $ready_count = 0;
            $unknown_count = 0;

            if ($wpdb->get_var($wpdb->prepare('SHOW TABLES LIKE %s', $table_name)) === $table_name) {
                $ready_count = (int) $wpdb->get_var("SELECT COUNT(*) FROM {$table_name} WHERE documents_scanned_at IS NOT NULL");
                $unknown_count = (int) $wpdb->get_var("SELECT COUNT(*) FROM {$table_name} WHERE documents_scanned_at IS NULL");
            }

            $workers = isset($state['workers']) && is_array($state['workers']) ? $state['workers'] : [];
            $processed = 0;
            ...
            return [
                'seed_status' => $state['seed_status'] ?? ($ready_count > 0 && $unknown_count === 0 ? 'ready' : 'empty'),
                'seed_started_at' => $state['seed_started_at'] ?? null,
                'seed_last_completed_at' => $state['seed_last_completed_at'] ?? null,
                'seed_last_error' => $state['seed_last_error'] ?? null,
                'worker_count' => (int) ($state['worker_count'] ?? self::DOCUMENT_SEED_WORKER_COUNT),
                'workers' => $workers,
                'processed' => $processed,
                'total_folders' => (int) ($state['total_folders'] ?? ($ready_count + $unknown_count)),
                'ready_count' => $ready_count,
                'unknown_count' => $unknown_count,
                'delta_status' => $state['delta_status'] ?? 'idle',
                'delta_started_at' => $state['delta_started_at'] ?? null,
                'delta_last_completed_at' => $state['delta_last_completed_at'] ?? null,
                'delta_last_error' => $state['delta_last_error'] ?? null,
                'delta_processed_items' => (int) ($state['delta_processed_items'] ?? 0),
                'delta_changed_folders' => (int) ($state['delta_changed_folders'] ?? 0),
                'delta_link' => $state['delta_link'] ?? null,
                'delta_next_url' => $state['delta_next_url'] ?? null,
            ];
        }

        public function start_document_seed(bool $reset = false): array
        {
            global $wpdb;

            self::install_schema();

            $folder_table = self::get_index_table_name();
            $total_folders = (int) $wpdb->get_var("SELECT COUNT(*) FROM {$folder_table}");
            if ($total_folders === 0) {
                throw new Metro_Registration_Graph_Exception('Build the trailer index before seeding document availability.', 400);
            }

            $state = get_option(self::DOCUMENT_SYNC_STATE_OPTION, []);
            if (!$reset && (($state['seed_status'] ?? null) === 'running' || ($state['delta_status'] ?? null) === 'running')) {
                return $this->get_document_sync_status();
            }

            $delta_link = $this->get_drive_root_delta_latest_link();
            $seed_started_at = current_time('mysql', true);
            $workers = [];
            foreach ($this->get_document_seed_worker_ids() as $worker_id) {
                $workers[$worker_id] = [
                    'worker_id' => $worker_id,
                    'status' => 'running',
                    'cursor' => '',
            ...
            $state = [
                ...
                'worker_count' => self::DOCUMENT_SEED_WORKER_COUNT,
                'workers' => $workers,
                'total_folders' => $total_folders,
                'delta_status' => 'idle',
                'delta_started_at' => null,
                'delta_last_completed_at' => $state['delta_last_completed_at'] ?? null,
                'delta_last_error' => null,
                'delta_processed_items' => 0,
                'delta_changed_folders' => 0,
                'delta_link' => $state['delta_link'] ?? null,
                'delta_next_url' => null,
                'seed_delta_catchup_url' => $delta_link,
            ];

            update_option(self::DOCUMENT_SYNC_STATE_OPTION, $state, false);

            return $this->get_document_sync_status();
        }

        public function process_document_seed_worker(int $worker_id): array
        {
            global $wpdb;

            $state = get_option(self::DOCUMENT_SYNC_STATE_OPTION, []);
            if (($state['seed_status'] ?? null) !== 'running') {
                return $this->get_document_sync_status();
            }

            $workers = isset($state['workers']) && is_array($state['workers']) ? $state['workers'] : [];
            if (!isset($workers[$worker_id]) || !is_array($workers[$worker_id])) {
                throw new Metro_Registration_Graph_Exception(sprintf('Document seed worker %d is not defined.', $worker_id), 500);
            }

            $worker = $workers[$worker_id];
            if (($worker['status'] ?? null) !== 'running') {
                return $this->get_document_sync_status();
            }
            ...
            $batch = $wpdb->get_results(
                $wpdb->prepare(
                    "SELECT folder_id
                     FROM {$folder_table}
                     WHERE MOD(CRC32(folder_id), %d) = %d
                       AND (documents_scanned_at IS NULL OR documents_scanned_at < %s)
                       AND folder_id > %s
                     ORDER BY folder_id ASC
                     LIMIT %d",
                    (int) ($state['worker_count'] ?? self::DOCUMENT_SEED_WORKER_COUNT),
                    $worker_id,
                    (string) $state['seed_started_at'],
                    (string) ($worker['cursor'] ?? ''),
                    self::DOCUMENT_SEED_BATCH_SIZE
                ),
                ARRAY_A
            ) ?: [];
            ...
            try {
                foreach ($batch as $row) {
                    ...
                    $this->get_trailer_documents_internal($folder_id, true);
                    $worker['cursor'] = $folder_id;
                    $worker['processed'] = (int) ($worker['processed'] ?? 0) + 1;
                }

                $worker['last_batch_completed_at'] = current_time('mysql', true);
                if (count($batch) < self::DOCUMENT_SEED_BATCH_SIZE) {
                    $worker['status'] = 'ready';
                }
            } catch (Throwable $exception) {
                $worker['status'] = 'error';
                $worker['last_error'] = $exception->getMessage();
                $state['seed_status'] = 'error';
                $state['seed_last_error'] = $exception->getMessage();
            ...
            $all_ready = true;
            foreach ($workers as $current_worker) {
                if (($current_worker['status'] ?? null) !== 'ready') {
                    $all_ready = false;
                    break;
                }
            }
            ...
            if ($all_ready) {
                $state['seed_status'] = 'catchup';
                $state['seed_last_completed_at'] = current_time('mysql', true);
                $state['delta_status'] = 'running';
                $state['delta_started_at'] = current_time('mysql', true);
                $state['delta_last_error'] = null;
                $state['delta_processed_items'] = 0;
                $state['delta_changed_folders'] = 0;
                $state['delta_next_url'] = $state['seed_delta_catchup_url'] ?? null;
            }
        ...
        public function start_document_delta_sync(bool $reset = false): array
        {
            $state = get_option(self::DOCUMENT_SYNC_STATE_OPTION, []);
            $status = $this->get_document_sync_status();

            if (($status['ready_count'] ?? 0) === 0) {
                throw new Metro_Registration_Graph_Exception('Seed document availability before running delta sync.', 400);
            }

            if (!$reset && ($state['delta_status'] ?? null) === 'running') {
                return $status;
            }

            $next_url = $state['delta_link'] ?? null;
            if (!$next_url) {
                $next_url = $this->get_drive_root_delta_latest_link();
            }
            ...
            $state['delta_processed_items'] = 0;
            $state['delta_changed_folders'] = 0;
            $state['delta_next_url'] = $next_url;
            update_option(self::DOCUMENT_SYNC_STATE_OPTION, $state, false);

            return $this->get_document_sync_status();
        }

        public function process_document_delta_sync_batch(): array
        {
            $state = get_option(self::DOCUMENT_SYNC_STATE_OPTION, []);
            if (($state['delta_status'] ?? null) !== 'running' || empty($state['delta_next_url'])) {
                return $this->get_document_sync_status();
            }

            try {
                $access_token = $this->get_access_token();
                $next_url = (string) $state['delta_next_url'];
                $pages_read = 0;
                $changed_folder_names = [];
                $latest_delta_link = $state['delta_link'] ?? null;

                while ($next_url && $pages_read < self::DOCUMENT_DELTA_PAGE_LIMIT) {
                    $payload = $this->graph_json($next_url, $access_token);
                    foreach ($payload['value'] ?? [] as $item) {
                        $folder_name = $this->extract_trailer_folder_from_delta_item($item);
                        if ($folder_name !== null) {
                            $changed_folder_names[$folder_name] = true;
                        }
                    }

                    $state['delta_processed_items'] = (int) ($state['delta_processed_items'] ?? 0) + count($payload['value'] ?? []);
                    $next_url = $payload['@odata.nextLink'] ?? null;
                    $latest_delta_link = $payload['@odata.deltaLink'] ?? $latest_delta_link;
                    $pages_read++;
                }

                $rescanned = $this->rescan_changed_trailer_folders(array_keys($changed_folder_names));
                $state['delta_changed_folders'] = (int) ($state['delta_changed_folders'] ?? 0) + $rescanned;
                $state['delta_next_url'] = $next_url;

                if (empty($next_url)) {
                    $state['delta_status'] = 'ready';
                    $state['delta_last_completed_at'] = current_time('mysql', true);
                    $state['delta_link'] = $latest_delta_link;
                    $state['seed_delta_catchup_url'] = null;
                    if (($state['seed_status'] ?? null) === 'catchup') {
                ...
                $state['delta_status'] = 'error';
                $state['delta_last_error'] = $exception->getMessage();
                update_option(self::DOCUMENT_SYNC_STATE_OPTION, $state, false);
                throw $exception;
            }
        }
        ...
        public function refresh_index_batch(bool $reset = false): array
        {
            ...
            try {
                ...
                if (empty($state['next_url'])) {
                    $this->delete_stale_folder_rows((string) ($state['started_at'] ?? current_time('mysql', true)));
                    $state['status'] = 'ready';
        ...
        public function refresh_bc_asset_index_batch(bool $reset = false): array
        {
            ...
            if ($reset || empty($state) || empty($state['next_url'])) {
                ...
                $state = [
                    ...
                    'total_count' => 0,
                    'next_offset' => 0,
                    'next_url' => $this->build_bc_assets_collection_url(0, true),
                ];
            ...
            try {
                ...
                $state['total_count'] = isset($payload['@odata.count']) ? (int) $payload['@odata.count'] : (int) ($state['total_count'] ?? 0);
                $state['next_offset'] = (int) ($state['next_offset'] ?? 0) + count($assets);

                if ($state['total_count'] > 0 && $state['next_offset'] < $state['total_count'] && count($assets) > 0) {
                    $state['next_url'] = $this->build_bc_assets_collection_url($state['next_offset'], false);
                } else {
                    $state['next_url'] = null;
                }

                if (empty($state['next_url'])) {
                    $this->delete_stale_bc_asset_rows((string) ($state['started_at'] ?? current_time('mysql', true)));
                    $state['status'] = 'ready';
                    $state['last_completed_at'] = current_time('mysql', true);
                } else {
                    $state['status'] = 'running';
        ...
        private function delete_stale_folder_rows(string $started_at): void
        {
            global $wpdb;
        ...
        private function delete_stale_bc_asset_rows(string $started_at): void
        {
            global $wpdb;
        ...
        private function get_trailer_documents_internal(string $folder_id, bool $force_refresh): array
        {
        ...
        private function get_drive_root_delta_latest_link(): string
        {
            $access_token = $this->get_access_token();
            $context = $this->resolve_storage_context($access_token);
            $payload = $this->graph_json(
                sprintf('/drives/%s/root/delta?token=latest', rawurlencode($context['drive']['id'])),
                $access_token
            );

            $delta_link = (string) ($payload['@odata.deltaLink'] ?? '');
            if ($delta_link === '') {
                throw new Metro_Registration_Graph_Exception('Graph did not return a drive delta link.', 502);
            }

            return $delta_link;
        }

        private function extract_trailer_folder_from_delta_item(array $item): ?string
        {
            $base_folder = strtolower(trim((string) $this->normalize_folder_path(
                $this->optional_config('METRO_SHAREPOINT_BASE_FOLDER') ?: self::DEFAULT_BASE_FOLDER
            ), '/'));
            $parent_path = (string) ($item['parentReference']['path'] ?? '');
            if ($parent_path === '') {
                return null;
            }

            $normalized = strtolower($parent_path);
            $needle = '/root:/' . $base_folder;
            $position = strpos($normalized, $needle);
            if ($position === false) {
                return null;
            }

            $relative = trim(substr($parent_path, $position + strlen($needle)), '/');
            if ($relative === '') {
                if (!empty($item['folder']) && !empty($item['name'])) {
            ...
            return trim((string) ($segments[0] ?? '')) ?: null;
        }

        private function rescan_changed_trailer_folders(array $folder_names): int
        {
            $folder_names = array_values(array_unique(array_filter(array_map('trim', $folder_names), 'strlen')));
            if (empty($folder_names)) {
                return 0;
            }

            $folder_names = array_slice($folder_names, 0, self::DOCUMENT_DELTA_RESCAN_LIMIT);
            $processed = 0;

            foreach ($folder_names as $folder_name) {
                $folder = $this->find_exact_folder_by_name($folder_name, true);
                if ($folder === null || empty($folder['id'])) {
                    continue;
                }

                $this->get_trailer_documents_internal((string) $folder['id'], true);
                $processed++;
            }
        ...
        private function build_unified_search_response(string $query, int $limit, array $asset_status, array $folder_status): array
        {
            $results = [];
            $used_folder_ids = [];
            $used_identifiers = [];
            ...
            if (($asset_status['row_count'] ?? 0) > 0) {
                $asset_rows = $this->lookup_asset_search_rows($query, $asset_status);
                $asset_matches = $this->score_asset_rows($asset_rows, $query);

                foreach ($asset_matches as $match) {
                    $folder = $this->find_best_folder_for_asset($match);
                    $result = $this->format_asset_result($match, $folder);
                    $results[] = $result;
                    $this->mark_trailer_result_identity($result, $used_folder_ids, $used_identifiers);
                }
            }

            if (($folder_status['row_count'] ?? 0) > 0) {
                $folder_rows = $this->lookup_search_rows($query, $folder_status);
                $folder_matches = $this->score_search_rows($folder_rows, $query);

                foreach ($folder_matches as $folder_match) {
                    $formatted_folder = $this->format_folder_result($folder_match);
                    $folder_id = (string) ($formatted_folder['id'] ?? '');
                    $folder_compact = $this->compact_search_value((string) ($formatted_folder['name'] ?? ''));

                    if ($folder_id !== '' && isset($used_folder_ids[$folder_id])) {
                        continue;
                    }

                    if ($folder_compact !== '' && isset($used_identifiers[$folder_compact])) {
                        continue;
                    }

                    $asset_match = null;
                    if ($folder_compact !== '' && ($asset_status['row_count'] ?? 0) > 0) {
                        $asset_match = $this->find_exact_asset_candidate((string) ($formatted_folder['name'] ?? ''));
                    }

                    if ($asset_match !== null) {
                        $result = $this->format_asset_result($asset_match, $folder_match);
                        $results[] = $result;
                        $this->mark_trailer_result_identity($result, $used_folder_ids, $used_identifiers);
                        continue;
                    }

                    $result = $this->format_legacy_folder_as_trailer($formatted_folder);
                    $results[] = $result;
                    $this->mark_trailer_result_identity($result, $used_folder_ids, $used_identifiers);
                }
            }
        ...
        private function mark_trailer_result_identity(array $result, array &$used_folder_ids, array &$used_identifiers): void
        {
            $folder = isset($result['folder']) && is_array($result['folder']) ? $result['folder'] : null;
            if ($folder && !empty($folder['id'])) {
                $used_folder_ids[(string) $folder['id']] = true;
            }

            foreach ([
                $result['trailer_number'] ?? '',
                $result['service_item_number'] ?? '',
                ($folder['name'] ?? ''),
            ] as $identifier) {
                $compact = $this->compact_search_value((string) $identifier);
                if ($compact !== '') {
                    $used_identifiers[$compact] = true;
                }
            }
        }
        ...
        private function fetch_business_central_exact_asset(string $query): ?array
        {
            $query = trim($query);
            if ($query === '') {
                return null;
            ...
            foreach ($fields as $field) {
                $asset = $this->fetch_business_central_exact_asset_by_field($field, $query);
                if ($asset !== null) {
                    return $asset;
                }
            }
        ...
        private function fetch_business_central_exact_asset_by_field(string $field, string $query): ?array
        {
            $access_token = $this->get_business_central_access_token();
            ...
            $url = sprintf(
                '/%s/ODataV4/%s/FixedAssets?$select=%s&$filter=%s eq %s&$top=1',
                $environment,
                $company_path,
                $select,
                rawurlencode($field),
                rawurlencode("'" . $escaped_query . "'")
            );

            try {
                $payload = $this->business_central_json($url, $access_token);
            ...
            return $this->build_bc_asset_index_row($row);
        }
        ...
        private function build_bc_assets_collection_url(int $skip = 0, bool $include_count = false): string
        {
            $environment = rawurlencode($this->required_config('METRO_BC_ENVIRONMENT'));
            $company_path = $this->build_business_central_company_segment($this->required_config('METRO_BC_COMPANY'));
            $select = implode(',', [
            ...
            $query = sprintf(
                '/%s/ODataV4/%s/FixedAssets?$select=%s&$orderby=No&$top=%d&$skip=%d',
                $environment,
                $company_path,
                $select,
        ...
        private function http_request(string $method, string $url, array $headers = [], ?string $body = null): array
        {
            ...
            do {
                $attempt++;
                $result = $this->http_request_once($method, $url, $headers, $body);
                if (!in_array($result['status'], [429, 500, 502, 503, 504], true) || $attempt >= self::HTTP_RETRY_ATTEMPTS) {
                    return $result;
                }

                $delay = min(20, (int) pow(2, $attempt - 1));
                sleep(max(1, $delay));
            } while ($attempt < self::HTTP_RETRY_ATTEMPTS);

            return $result;
        }

        private function http_request_once(string $method, string $url, array $headers = [], ?string $body = null): array
        {
        ...
        private function build_trailer_manifest_lookup(array $trailers): array
        {
            $lookup = [
                'by_folder_id' => [],
                'by_identifier' => [],
            ...
            foreach ($trailers as $trailer) {
                ...
                $normalized = $this->normalize_manifest_trailer_row($trailer);
                if ($normalized === null) {
                    continue;
                }

                if ($normalized['folder_id'] !== '') {
                    $lookup['by_folder_id'][$normalized['folder_id']] = $normalized;
                }

                foreach ([
                    $normalized['trailer_number'],
                    $normalized['service_item_number'],
                ] as $identifier) {
                    $compact = $this->compact_search_value($identifier);
                    if ($compact !== '' && !isset($lookup['by_identifier'][$compact])) {
                        $lookup['by_identifier'][$compact] = $normalized;
                    }
                }
            }
        ...
        private function normalize_manifest_trailer_row(array $trailer): ?array
        {
            $folder = isset($trailer['folder']) && is_array($trailer['folder']) ? $trailer['folder'] : [];
            $trailer_number = trim((string) ($trailer['trailer_number'] ?? ''));
            $service_item_number = trim((string) ($trailer['service_item_number'] ?? ''));

            if ($trailer_number === '' && $service_item_number === '' && empty($folder['id'])) {
                return null;
            }

            return [
                'folder_id' => trim((string) ($folder['id'] ?? '')),
                'folder_name' => trim((string) ($folder['name'] ?? '')),
                'trailer_number' => $trailer_number,
                'service_item_number' => $service_item_number,
                'description' => trim((string) ($trailer['description'] ?? '')),
                'make' => trim((string) ($trailer['make'] ?? '')),
                'vehicle_year' => trim((string) ($trailer['vehicle_year'] ?? '')),
                'serial_vin' => trim((string) ($trailer['serial_vin'] ?? '')),
                'registration_number' => trim((string) ($trailer['registration_number'] ?? '')),
            ];
        }

        private function build_manifest_row_for_folder(array $folder, array $documents, array $manifest_lookup): array
        {
            $folder_id = (string) ($folder['id'] ?? '');
            $folder_name = (string) ($folder['name'] ?? '');
            $trailer = $manifest_lookup['by_folder_id'][$folder_id]
                ?? $manifest_lookup['by_identifier'][$this->compact_search_value($folder_name)]
                ?? $this->find_manifest_trailer_from_business_central($folder_name);

            $document_names = array_map(static function (array $document): string {
                return (string) ($document['name'] ?? '');
            }, $documents);

            return [
                'trailer_number' => $trailer['trailer_number'] ?? $folder_name,
                'service_item_number' => $trailer['service_item_number'] ?? '',
                'description' => $trailer['description'] ?? '',
                'make' => $trailer['make'] ?? '',
                'vehicle_year' => $trailer['vehicle_year'] ?? '',
                'serial_vin' => $trailer['serial_vin'] ?? '',
                'registration_number' => $trailer['registration_number'] ?? '',
                'has_registration_pdf' => $this->has_document_type($documents, 'Registration') ? 'Yes' : 'No',
                'has_fhwa_pdf' => $this->has_document_type($documents, 'FHWA Inspection') ? 'Yes' : 'No',
                'document_count' => count($documents),
                'documents' => implode(' | ', array_filter($document_names, 'strlen')),
            ];
        }

        private function find_manifest_trailer_from_business_central(string $identifier): ?array
        {
            $match = $this->find_exact_asset_candidate($identifier);
            if ($match === null) {
                return null;
            }
            ...
            return [
                'folder_id' => '',
                'folder_name' => '',
                'trailer_number' => trim((string) ($match['asset_no'] ?? '')),
                'service_item_number' => trim((string) ($match['service_item_number'] ?? '')),
                'description' => trim((string) ($match['description'] ?? '')),
                'make' => trim((string) ($match['make'] ?? '')),
                'vehicle_year' => trim((string) ($match['vehicle_year'] ?? '')),
                'serial_vin' => trim((string) ($match['serial_vin'] ?? '')),
                'registration_number' => trim((string) ($match['registration_number'] ?? '')),
            ];
        }

        private function write_manifest_csv(string $path, array $rows): void
        {
            $handle = fopen($path, 'wb');
            if ($handle === false) {
                throw new Metro_Registration_Graph_Exception('Could not create the trailer manifest CSV.', 500);
            }
            ...
            try {
                ...
                foreach ($rows as $row) {
                    fputcsv($handle, [
                        $row['trailer_number'] ?? '',
                        $row['service_item_number'] ?? '',
                        $row['description'] ?? '',
                        $row['make'] ?? '',
                        $row['vehicle_year'] ?? '',
                        $row['serial_vin'] ?? '',
                        $row['registration_number'] ?? '',
                        $row['has_registration_pdf'] ?? '',
                        $row['has_fhwa_pdf'] ?? '',
                        $row['document_count'] ?? '',
                        $row['documents'] ?? '',
                    ]);
