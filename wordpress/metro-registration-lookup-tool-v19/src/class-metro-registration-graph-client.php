...
if (!class_exists('Metro_Registration_Graph_Client')) {
    class Metro_Registration_Graph_Client
    {
        ...
        public function search_trailer_folders(string $query, int $limit = self::SEARCH_LIMIT_DEFAULT): array
        {
            ...
            $response = $this->build_folder_search_response($query, $limit, $status);

            if ($response['total_matches'] === 0) {
                $numeric_fallback = $this->build_numeric_search_fallback($query, 4);
                if ($numeric_fallback !== null) {
                    $fallback_response = $this->build_folder_search_response($numeric_fallback, $limit, $status);
            ...
            if (($status['row_count'] ?? 0) === 0 && $response['total_matches'] === 0) {
                throw new Metro_Registration_Graph_Exception(
        ...
        public function lookup_exact_trailer_batch(array $queries): array
        {
            ...
            if ($status['row_count'] > 0) {
                ...
                foreach ($queries as $query) {
                    $lookup_candidates[] = $query['query'];
                    if (!empty($query['numeric_fallback'])) {
                        $lookup_candidates[] = $query['numeric_fallback'];
                    }
                }

                $normalized_values = array_values(array_unique(array_map('strtoupper', $lookup_candidates)));
                $compact_values = array_values(array_unique(array_map([$this, 'compact_search_value'], $lookup_candidates)));
            ...
            foreach ($queries as $query) {
                $matched_row = $this->lookup_exact_folder_candidate(
                    $query['query'],
                    $rows_by_normalized,
                    $rows_by_compact
                );
