...
export const rentalBillingFacts = pgTable(
  ...
  (table) => ({
    sourceRowUnique: uniqueIndex("rental_billing_facts_source_row_unique").on(
    ...
    customerActivityIdx: index("rental_billing_facts_customer_activity_idx").on(
      table.customerNumber,
      table.postingDate,
    ),
    leaseActivityIdx: index("rental_billing_facts_lease_activity_idx").on(
      table.leaseKey,
      table.postingDate,
    ),
    invoiceLineIdx: index("rental_billing_facts_invoice_line_idx").on(
      table.documentNo,
      table.lineNo,
    ),
  }),
...
export const rentalInvoiceFacts = pgTable(
  ...
  (table) => ({
    sourceRowUnique: uniqueIndex("rental_invoice_facts_source_row_unique").on(
      table.sourceProvider,
      table.sourceRowId,
    ),
    sourceDocUnique: uniqueIndex("rental_invoice_facts_source_doc_unique").on(
      table.sourceProvider,
      table.documentNo,
    ),
    customerActivityIdx: index("rental_invoice_facts_customer_activity_idx").on(
      table.customerNumber,
      table.postingDate,
    ),
...
export const arLedgerFacts = pgTable(
  "ar_ledger_facts",
  {
    id: text().primaryKey(),
    sourceProvider: text().notNull(),
    ...
    remainingAmount: numeric({ precision: 18, scale: 2 }).default("0").notNull(),
  ...
  (table) => ({
    ...
    documentIdx: index("ar_ledger_facts_document_idx").on(table.documentNo),
  }),
);
...
export const entityActivityFacts = pgTable(
  "entity_activity_facts",
  {
    ...
    activityType: text().notNull(),
    activityAt: timestamp({ withTimezone: true }),
    title: text().notNull(),
    subtitle: text(),
  ...
  (table) => ({
    entityActivityIdx: index("entity_activity_facts_entity_activity_idx").on(
      table.entityType,
      table.entityId,
      table.activityAt,
    ),
    relatedIdx: index("entity_activity_facts_related_idx").on(
...
export const projectionEvents = pgTable(
  ...
  (table) => ({
    statusCreatedIdx: index("projection_events_status_created_idx").on(
      table.status,
      table.createdAt,
    ),
    sourceIdx: index("projection_events_source_idx").on(table.sourceTable, table.sourceId),
  }),
);

export const projectionWatermarks = pgTable("projection_watermarks", {
  projectionName: text().primaryKey(),
  lastEventId: text(),
  lastProcessedAt: timestamp({ withTimezone: true }),
  projectionVersion: text().default("0").notNull(),
  status: text().default("idle").notNull(),
  errorMessage: text(),
  updatedAt: timestamp({ withTimezone: true }).defaultNow().notNull(),
});

export const equipmentDetailSummary = pgTable(
  "equipment_detail_summary",
  {
    ...
    leaseCount: integer().default(0).notNull(),
    lifetimeRevenue: numeric({ precision: 18, scale: 2 }).default("0").notNull(),
    firstBilledAt: timestamp({ withTimezone: true }),
    latestBilledAt: timestamp({ withTimezone: true }),
    latestInvoiceNo: text(),
    latestLeaseKey: text(),
    latestCustomerNo: text(),
    latestCustomerName: text(),
  ...
  (table) => ({
    assetNumberIdx: index("equipment_detail_summary_asset_number_idx").on(
      table.assetNumber,
    ),
    activityIdx: index("equipment_detail_summary_activity_idx").on(table.latestBilledAt),
  }),
);

export const customerDetailSummary = pgTable(
  "customer_detail_summary",
  {
    ...
    portalEnabled: boolean().default(false).notNull(),
    blocked: boolean().default(false).notNull(),
    locations: jsonb().$type<Array<Record<string, unknown>>>().default([]).notNull(),
    contractCount: integer().default(0).notNull(),
    invoiceCount: integer().default(0).notNull(),
    leaseCount: integer().default(0).notNull(),
    equipmentCount: integer().default(0).notNull(),
    ...
    firstInvoiceDate: timestamp({ withTimezone: true }),
    latestInvoiceDate: timestamp({ withTimezone: true }),
    latestActivityDate: timestamp({ withTimezone: true }),
    sourceProvider: text(),
    sourceExternalId: text(),
    sourcePayloadAvailable: boolean().default(false).notNull(),
    searchText: text().notNull(),
  ...
  (table) => ({
    numberIdx: uniqueIndex("customer_detail_summary_number_idx").on(
      table.customerNumber,
    ),
    activityIdx: index("customer_detail_summary_activity_idx").on(
      table.latestActivityDate,
    ),
  }),
);

export const equipmentCustomerHistory = pgTable(
  ...
  (table) => ({
    assetIdx: index("equipment_customer_history_asset_idx").on(
      table.assetNumber,
      table.latestBilledAt,
    ),
    customerIdx: index("equipment_customer_history_customer_idx").on(
      table.customerNumber,
      table.latestBilledAt,
    ),
  }),
...
export const customerEquipmentHistory = pgTable(
  ...
  (table) => ({
    customerIdx: index("customer_equipment_history_customer_idx").on(
      table.customerNumber,
      table.latestBilledAt,
    ),
    assetIdx: index("customer_equipment_history_asset_idx").on(table.assetNumber),
  }),
);

export const leaseEquipmentSummary = pgTable(
  "lease_equipment_summary",
  {
    id: text().primaryKey(),
    leaseKey: text().notNull(),
    customerNumber: text(),
    customerName: text(),
    assetId: text(),
    assetNumber: text(),
  ...
  (table) => ({
    leaseIdx: index("lease_equipment_summary_lease_idx").on(
      table.leaseKey,
      table.latestBilledAt,
    ),
    assetIdx: index("lease_equipment_summary_asset_idx").on(table.assetNumber),
  }),
);

export const invoiceLineRegister = pgTable(
  ...
  (table) => ({
    docLineUnique: uniqueIndex("invoice_line_register_doc_line_unique").on(
      table.sourceProvider,
      table.documentNo,
      table.lineNo,
    ),
    invoiceIdx: index("invoice_line_register_invoice_idx").on(
      table.documentNo,
      table.lineNo,
    ),
    assetActivityIdx: index("invoice_line_register_asset_activity_idx").on(
      table.assetNumber,
      table.postingDate,
    ),
    customerActivityIdx: index("invoice_line_register_customer_activity_idx").on(
      table.customerNumber,
      table.postingDate,
    ),
    leaseActivityIdx: index("invoice_line_register_lease_activity_idx").on(
      table.leaseKey,
      table.postingDate,
    ),
  }),
...
export const docusealTemplateClassifications = pgTable(
  "docuseal_template_classifications",
  {
    ...
    customerRequiredFields: jsonb().$type<string[]>().default([]).notNull(),
    active: boolean().default(true).notNull(),
    createdAt: timestamp({ withTimezone: true }).defaultNow().notNull(),
    updatedAt: timestamp({ withTimezone: true }).defaultNow().notNull(),
  },
